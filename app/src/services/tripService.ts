...
export const createTrip = async (userId: string, tripData: {
    title: string; start_date: string; end_date: string; image: string; budget: number; family_id?: string | null;
}): Promise<string> => {
...
export const addTripItem = async (table: string, data: Record<string, unknown>): Promise<string> => {
    const { data: row, error } = await db.from(table).insert(data).select('id').single();
    if (error) throw error;
    return row.id;
};
