        ...
        <div className="min-h-screen bg-[var(--color-bg-primary)] flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-md">
                <div className="text-center mb-8">
                    ...
                    <h1 className="cx-h1 text-[var(--color-text-primary)]">Join a Family</h1>
                    <p className="text-[var(--color-text-secondary)] mt-2">Enter the invite code shared by your family member</p>
                </div>
                ...
                {!found ? (
                    <form onSubmit={handleLookup} className="space-y-4">
                        <input
                            ...
                            maxLength={6}
                            className="w-full bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-4 text-[var(--color-text-primary)] text-center text-2xl tracking-widest font-mono focus:outline-none focus:border-brand-teal uppercase"
                            required
                        />
                        <Button type="submit" variant="primary" size="lg" disabled={lookupLoading || code.length < 6}>
                            {lookupLoading ? <Loader2 className="animate-spin" /> : 'Look Up Code'}
                        </Button>
                    </form>
                ) : user ? (
                    <Panel raked className="p-6 text-center">
                        <PartyPopper size={32} className="text-brand-teal mx-auto mb-3" />
                        <h2 className="cx-h2 text-[var(--color-text-primary)] mb-2">{found.familyName}</h2>
                        <p className="text-[var(--color-text-secondary)] mb-6">Would you like to join this family group?</p>
                        <div className="flex gap-3">
                            <Button type="button" variant="outline" size="lg" onClick={() => setFound(null)}>
                                Cancel
                            </Button>
                            <Button type="button" variant="primary" size="lg" onClick={handleJoin} disabled={joinLoading}>
                                {joinLoading ? <Loader2 className="animate-spin" /> : 'Join Family'}
                            </Button>
                        </div>
                    </Panel>
                ) : (
                    <Panel raked className="p-6">
                        <div className="text-center mb-6">
                            <PartyPopper size={32} className="text-brand-teal mx-auto mb-3" />
                            <h2 className="cx-h2 text-[var(--color-text-primary)] mb-1">You're invited to {found.familyName}</h2>
                        ...
                        <form onSubmit={handleEmailAuth} className="space-y-3">
                            ...
                            <input
                                ...
                                required
                            />
                            <Button type="submit" variant="primary" size="lg" disabled={authLoading || joinLoading}>
                        ...
                        <button
                            ...
                            className="w-full mt-3 py-2 text-sm text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] transition-colors"
                        >
